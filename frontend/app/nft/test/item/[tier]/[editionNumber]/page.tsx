...
import { api, formatUsd, formatZec, nftItemLabel, type NftItem } from "@/lib/api";
...
export default function NftItemPage() {
  ...
  // Brai, 2026-09-19: editionNumber is scoped per tier now -- the URL is
  // /nft/test/item/<tier>/<editionNumber>, see api.ts's nftItemPath.
  const tierSlugParam = typeof params.tier === "string" ? params.tier.toLowerCase() : "";
  const tierSlug = VALID_TIER_SLUGS.has(tierSlugParam) ? (tierSlugParam as "papiro" | "fragmento" | "reliquia") : null;
  const { t } = useLanguage();
  ...
  // Brai, 2026-09-19: "cuando pones buy now en un nft listado no aparece el
  // wait" -- same fix as the mint page's isSubmitting: generating the
  // purchase address for a real ZEC buy can take 30-40s, and without a
  // loading state the button just sits there looking broken the whole time.
  ...
  async function reload() {
    if (!tierSlug) return;
    try {
      const r = await api.getNftItem(COLLECTION_SLUG, tierSlug, editionNumber);
      setItem(r.item);
  ...
  useEffect(() => {
    if (!Number.isFinite(editionNumber) || !tierSlug) return;
    reload();
    api.getMode().then((m) => setIsRealMode(m.currencies.ZEC.mode === "real")).catch(() => {});
  }, [editionNumber, tierSlug]);
      ...
      <div className="nft-item-layout">
        ...
        <div className="card nft-item-panel">
          <h1 style={{ marginTop: 0 }}>
            {item.name ?? nftItemLabel(item.tier, item.editionNumber)}{" "}
